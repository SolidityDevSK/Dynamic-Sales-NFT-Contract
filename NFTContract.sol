...
contract NFTContract is ERC721Enumerable, Ownable {
  ...
  constructor(string memory _baseURI) ERC721("NFTContract", "NFTContract") {
    baseURI = _baseURI;
  }
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    if(totalFreeAmount < maxFreeAmount){
      ...
      if(nowRoundSaleAmount + _mintAmount <= nowRoundSaleMaxAmount*rate){
        ...
        amountOfPayment = (remaining * (rate +1) * pricePerNFT) + (_mintAmount - remaining) * pricePerNFT * rate;
  ...
  function doHaveFree(address _add) public view returns(uint256){
